...
data_hline <- data.frame(NAME = unique(pseudo$NAME),  # Create data for lines
                         hline = c(0, 0, 1))

# first, plot
av_plot <- ggplot(pseudo, aes(x = 1, y = BIAS, group = NAME)) +
...
ggplot(tonic_diff) +
  ...
  geom_point(aes(0, DIFF), size = 5, colour = blues[7]) +
  geom_linerange(aes(0, ymin = DIFF-ci, ymax = DIFF+ci), colour = blues[7], size = 1) +
 #xlim(-.025,.025) +
  ylim(-.05,.05) +
  labs(x = '', y = 'Difference (Block 2 - Block 1)') +
  scale_x_continuous(breaks = 0, labels = 'SUM') +
  theme_classic() +
...
# saving
ggsave('TOTwTONE-plot.png', tonic_all, device = NULL, path = aPath,
       width = 9, height = 5, dpi = 600)
```
...
ggplot(phasic_diff) +
  geom_hline(aes(yintercept = 0), colour = 'gray50') +
  geom_point(aes(0, DIFF), size = 5, colour = grees[7]) +
  geom_linerange(aes(0, ymin = DIFF-ci, ymax = DIFF+ci), colour = grees[7], size = 1) +
 #xlim(-.025,.025) +
  ylim(-.05,.05) +
...
# saving
ggsave('TONE-plot.png', phasic_all, device = NULL, path = aPath,
       width = 9, height = 5, dpi = 600)
```
...
# linear regression of interaction
EW_DV$tone <- as.factor(EW_DV$tone)
EW_DV$block <- as.factor(EW_DV$block)

model.EWS <- lm(EWS ~ tone*block, data = EW_DV)
summary(model.EWS)
```
...
ggplot(tonic, aes(group = ID, colour = as.factor(block), fill = as.factor(block))) +
  ...
  scale_x_discrete(breaks = c(1,2), labels = c('Early', 'Late')) +
  labs(title = 'Bias: time on task', x = '', y = '') +
  ylim(-.12,.12) +
  theme_classic() +
  theme(legend.position = 'none',
        axis.title = element_text(size = 14),
...
ggplot(phasic_bias, aes(group = ID, colour = as.factor(tone), fill = as.factor(tone))) +
  ...
  scale_x_discrete(breaks = c(1,2), labels = c('No tone', 'Tone')) +
  labs(title = 'Bias: tone effect', x = '', y = 'EWB') +
  ylim(-.12,.12) +
  theme_classic() +
  theme(legend.position = 'none',
        axis.title = element_text(size = 14),
...
# combine plots
EWB_plot <- ggarrange(phasic_bias_plot, tonic_bias_plot,
                      ncol = 2, nrow = 1)
EWB_plot
...
# Bias difference - in tone trials
```{r}
# now plot difference and put together!
# summary of difference
phasic_bias_diff <- summarySE(data = phasic_bias_wide, measurevar = 'DIFF')
# calculate effect size
phasic_bias_diff <- phasic_bias_diff %>% mutate(es = DIFF/sd)

ggplot(phasic_bias_diff) +
  geom_hline(aes(yintercept = 0), colour = 'gray50') +
  geom_point(aes(0, DIFF), size = 5, colour = purps[7]) +
  geom_linerange(aes(0, ymin = DIFF-ci, ymax = DIFF+ci), colour = purps[7], size = 1) +
 #xlim(-.025,.025) +
  ylim(-.05,.05) +
  labs(x = '', y = 'Difference (Tone - No tone)') +
  scale_x_continuous(breaks = 0, labels = 'BIAS') +
  theme_classic() +
  theme(axis.title = element_text(size = 14),
        axis.text = element_text(size = 12))-> phasic_bias_diff_plot

phasic_bias_all <- ggarrange(phasic_bias_plot, phasic_bias_diff_plot,
          ncol = 2, nrow = 1,
          widths = c(1, 0.5))
...
# saving
ggsave('TONE_BIAS-plot.png', phasic_bias_all, device = NULL, path = aPath,
       width = 9, height = 5, dpi = 600)
```
...
ggsave('Interact_bias_plot.png', inter_bias_plot, device = NULL, path = aPath,
       width = 6, height = 4, dpi = 600)

# linear regression
model.EWB <- lm(EWB ~ tone*block, data = EW_DV)
summary(model.EWB)

...
# there are 10 participants removed at this stage
...
all_dat <- bisectData[, c(224,11,12,289,211,212,274,275,214,36,37,46,89,164,165,206,207,
                          208,216:218,222,225,228,229,237,251,352,356,319,321,386,388)]
...
all_dat <- merge(qualData, all_dat, by = 'prolific_id')
...
```
So far, 11 participants have been removed from the data
...
length(ncalib$prolific_id) # yes, there are 123 prolific ids here
...
# this step removed 11 participants from data-set
...
# remove NAs
CALFIT <- dplyr::filter(CALFIT, !is.na(LOC_CLK))
...
# recode ID
CALFIT <- CALFIT[order(CALFIT$prolific_id) ,]
CALFIT$ID <- as.character(as.numeric(CALFIT$prolific_id))
CALFIT$ID <- as.factor(CALFIT$ID)

for(ID in levels(CALFIT$ID)){
    tmp <- CALFIT[CALFIT$ID == ID, c("LOC_CLK","LOC_DOT",'prolific_id')]
...
There are no participants that fail the Calibration stage - after NAs and mid points are removed! Hurrah 
...
all_dat <- filter(all_dat, !(prolific_id %in% cal_x))

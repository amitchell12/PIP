...
ntrials <- ntrials[order(ntrials$prolific_id) ,] # order by prolific id for easy identification
# this is the actual number of participants recruited
length(ntrials$prolific_id) 
...
sanity_check <- aggregate(bisect_error ~ pip*block, length, data = all_dat)
...
nvalid <- aggregate(bisect_x~prolific_id, length, data=all_dat)
length(nvalid$prolific_id)
```
...
# remove NAs
#CALFIT2 <- dplyr::filter(CALFIT2, !is.na(LOC_CLK))
...
for(ID in levels(CALFIT2$ID)){
    tmp <- CALFIT2[CALFIT2$ID == ID, c("LOC_CLK","LOC_DOT",'prolific_id')]
